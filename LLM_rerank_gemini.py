#!/usr/bin/env python3
"""
LLM Re-ranking for Recommendation Systems
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description='LLM Re-ranking for Recommendation Systems')
...
def get_item_details_for_prompt(item_asin: str, meta_lookup_dict: Dict) -> Tuple[str, str, str]:
    ...
    if item_asin in meta_lookup_dict:
        ...
        return f"Unknown Item (ASIN: {item_asin})", "N/A Brand", "N/A Category"
...
{user_history}
The user history shows the items the user has interacted in chronological order.
The first item is the oldest one the user interacted, and the last item is the most recent one.
Each entry follows this format: [Position]. [Item Title] [Brand] [Categories] - Rating: [User Rating]
...
def process_users(rec_list: Dict, train_df: pd.DataFrame, meta_data_dict: Dict, 
                 model_instance, args, history_count: int) -> List[Dict]:
    ...
    system_instruction = """You are a recommendation system. Given user history in chronological order, recommend an item from the candidate pool with its index letter."""
    ...
    for user_index, (user_id_str, original_candidate_asins) in enumerate(user_list):
        ...
        if processed_user_count % 10 == 0:
            ...
            temp_df.to_csv(f'dataset/{args.data_name}/gemini_reranked_responses_{start_idx}_{end_idx}.csv', index=False)
...
def main():
    ...
    # Process users
    all_reranking_calls = process_users(filtered_rec_list, train_df, meta_data_dict, model_instance, args, history_count)
    ...
    if all_reranking_calls:
        ...
        results_file = f'dataset/{args.data_name}/gemini_reranked_responses_{start_idx}_{end_idx}.csv'
        results_df.to_csv(results_file, index=False)
        print(f"Saved detailed results to {results_file}")
        
        # Save token summary
        token_file = f'dataset/{args.data_name}/token_summary_{start_idx}_{end_idx}.csv'
        save_token_summary(results_df, token_file)
        
        # Create ensemble predictions
        ensemble_df = ensemble_predictions(results_df)
        prediction_file = f'dataset/{args.data_name}/gemini_reranked_prediction_{start_idx}_{end_idx}.csv'
