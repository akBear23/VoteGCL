#!/usr/bin/env python3
"""
LLM Re-ranking for Recommendation Systems with Few-Shot Prompting using OpenAI GPT API
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description='LLM Re-ranking for Recommendation Systems with Few-Shot Prompting using GPT')
    ...
    parser.add_argument('--model_file', type=str, default=None,
                        help='Path to saved model file for user similarity (optional, auto-detected if not provided)')
    
    return parser.parse_args()


def find_model_file(data_name: str) -> Optional[str]:
    """Automatically find a saved model file in the dataset folder."""
    dataset_folder = f'dataset/{data_name}'
    ...
    for pattern in model_patterns:
        full_pattern = os.path.join(dataset_folder, pattern)
    ...
    print(f"Warning: No model file found in '{dataset_folder}' using common patterns")
    return None
...
def create_system_message() -> str:
    """Create system message for GPT with few-shot prompting context."""
    return """You are a recommendation system expert. Given a user's interaction history, a list of candidate items, and a reference example from a similar user, your task is to reorder the candidates based on the user's preferences. Use the reference example to understand behavioral patterns and improve your ranking accuracy."""
...
def create_user_prompt_fewshot(user_history: str, candidate_list: str, candidate_count: int,
    ...
    """Create user prompt for GPT with few-shot learning examples."""
...
def process_users(rec_list: Dict, train_df: pd.DataFrame, meta_data_dict: Dict, 
                 client: OpenAI, args, history_count: int, user_similarities: Dict) -> List[Dict]:
    ...
    for user_index, (user_id_str, original_candidate_asins) in enumerate(user_list):
        ...
        if processed_user_count % 10 == 0:
            ...
            temp_df.to_csv(f'dataset/{args.data_name}/gpt_fewshot_reranked_responses_{start_idx}_{end_idx}.csv', index=False)
...
def create_augmented_train_file(ensemble_df: pd.DataFrame, data_name: str, default_rating: float = 4.0):
    ...
    augmented_train_file = f'dataset/{data_name}/train_augmented_gpt_fewshot.txt'
...
def main():
    ...
    # Load saved model for user similarity (auto-detect if not provided)
    model_file = args.model_file
    if not model_file:
        model_file = find_model_file(args.data_name)
    
    saved_model = load_saved_model(model_file)
    ...
    all_reranking_calls = process_users(filtered_rec_list, train_df, meta_data_dict, client, args, history_count, user_similarities)
    ...
    if all_reranking_calls:
        ...
        results_file = f'dataset/{args.data_name}/gpt_fewshot_reranked_responses_{start_idx}_{end_idx}.csv'
        results_df.to_csv(results_file, index=False)
        print(f"Saved detailed results to {results_file}")
        
        # Save token summary
        token_file = f'dataset/{args.data_name}/token_summary_gpt_fewshot_{start_idx}_{end_idx}.csv'
        save_token_summary(results_df, token_file)
        
        # Create ensemble predictions
        ensemble_df = ensemble_predictions(results_df)
        prediction_file = f'dataset/{args.data_name}/gpt_fewshot_reranked_prediction_{start_idx}_{end_idx}.csv'
        ...
        print(f"\nGPT few-shot processing complete! Processed {len(results_df) // args.api_calls_per_user} users with {len(results_df)} total API calls.")
    else:
