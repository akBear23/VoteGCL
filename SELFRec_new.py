...
class SELFRec(object):
    def __init__(self, config):
        ...
        # Add support for augmented data
        if config.contain('augmented.set'):
            augmented_data = FileIO.load_data_set(config['augmented.set'], config['model']['type'])
            self.kwargs['augmented_set'] = augmented_data
